...
----------------------------------------------------------------------
----------------------------------------------------------------------
local function get_smart_next_grid_division_F(next_grid_division, state)
  _, _, up_number_signature, _, _ = reaper.TimeMap2_timeToBeats(proj, next_grid_division)
  Msg(up_number_signature)

  next_grid_division_cut = asm_math.cutFloatTo(next_grid_division, 2)
  count_next_grid_division = 1
  --[[if state ~= 4 then
  ...
  while next_grid_division_cut < p_max do
    ...
    next_grid_divison_cut = asm_math.cutFloatTo(next_grid_division, 2)
  ...
  if min_position ~= math.huge and max_position ~= math.huge then
   ...
   --[[ if count_next_grid_division == 1 then
    ...
    if c_min == p_min and c_max == p_max then    
      ...
      next_grid_division = get_smart_next_grid_division_F(next_grid_division, state)
