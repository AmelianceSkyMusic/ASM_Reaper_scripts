...
--[[local conf_var = {
 --[[ stop_txt, pause_txt, play_txt, rec_txt, --texts

  R_stop_bg,    G_stop_bg,    B_stop_bg,   --Stop color BG (RGB)
  R_pause_bg,   G_pause_bg,   B_pause_bg,  --Pause color BG (RGB)
  R_play_bg,    G_play_bg,    B_play_bg,   --Play color BG (RGB)
  R_rec_bg,     G_rec_bg,     B_rec_bg,    --Rec color BG (RGB)

  R_stop_txt,   G_stop_txt,   B_stop_txt,  --Stop color TXT (RGB)
  R_pause_txt,  G_pause_txt,  B_pause_txt, --Pause color TXT (RGB)
  R_play_txt,   G_play_txt,   B_play_txt,  --Play color TXT (RGB)
  R_rec_txt,    G_rec_txt,    B_rec_txt,   --Rec color TXT (RGB)

  repeat_unique_color_state, -- enable repeat uniqur color (below)
  repeat_on_color_state,     -- enable repeat ON uniqur color
  repeat_off_color_state,    -- enable repeat OFF uniqur color
  R_repeat_on, G_repeat_on, B_repeat_on,     -- Repeat color ON
  R_repeat_off, G_repeat_off, B_repeat_off,  -- Repeat color OFF

  user_record_key_number, -- * (num keyboard)
  user_repeat_key_number, -- / (num keyboard)
  docker_start, --start in docker

  repeat_indicate_state    -- eneble repeat indication]]
  --[[stop_txt = 'STOP', pause_txt = 'PAUSE', play_txt = 'PLAY', rec_txt = 'RECORD', --texts
  
  R_stop_bg = 30,     G_stop_bg = 30,     B_stop_bg = 30, --Stop color BG (RGB)
  R_pause_bg = 30,    G_pause_bg = 30,    B_pause_bg = 30, --Pause color BG (RGB)
  R_play_bg = 30,     G_play_bg = 30,     B_play_bg = 30, --Play color BG (RGB)
  R_rec_bg = 200,     G_rec_bg = 70,      B_rec_bg = 70, --Rec color BG (RGB)
  
  R_stop_txt = 100,   G_stop_txt = 100,   B_stop_txt = 100, --Stop color TXT (RGB)
  R_pause_txt = 100,  G_pause_txt = 170,  B_pause_txt = 190, --Pause color TXT (RGB)
  R_play_txt = 75,    G_play_txt = 170,   B_play_txt = 75, --Play color TXT (RGB)
  R_rec_txt = 30,     G_rec_txt = 30,     B_rec_txt = 30, --Rec color TXT (RGB)
  ...
  R_repeat_on = 100, G_repeat_on = 170, B_repeat_on = 190, -- Repeat color ON
  R_repeat_off = 35, G_repeat_off = 35, B_repeat_off = 35, -- Repeat color OFF
...
function save_settings()
  --settings_file_check = asm_io.file_exists(settings_file_path)
  --if settings_file_check == false and file_is_on_start == 1 then
  --  Msg(01)
  --else
  --Msg(data_to_settings_file)
    settings_file = io.open (settings_file_path, 'w') -- open file for write
    --Msg(settings_file)
    Msg(settings_file_path)
    ...
    data_to_settings_file = 
                            ...
                            conf_var.R_stop_txt..'\n'..   conf_var.G_stop_txt..'\n'..   conf_var.B_stop_txt..'\n'..
                            conf_var.R_pause_txt..'\n'..  conf_var.G_pause_txt..'\n'..  conf_var.B_pause_txt..'\n'..
                            conf_var.R_play_txt..'\n'..   conf_var.G_play_txt..'\n'..   conf_var.B_play_txt..'\n'..
                            conf_var.R_rec_txt..'\n'..    conf_var.G_rec_txt..'\n'..    conf_var.B_rec_txt..'\n'..
    
                            conf_var.repeat_unique_color_state..'\n'..
                            conf_var.repeat_on_color_state..'\n'..
                            conf_var.repeat_off_color_state..'\n'..
                            conf_var.R_repeat_on..'\n'..   conf_var.G_repeat_on..'\n'..   conf_var.B_repeat_on..'\n'..
                            conf_var.R_repeat_off..'\n'..  conf_var.G_repeat_off..'\n'..  conf_var.B_repeat_off..'\n'..
    
                            conf_var.user_record_key_number..'\n'..
                            conf_var.user_repeat_key_number..'\n'..
                            
                            conf_var.docker_start..'\n'..
                            
                            conf_var.repeat_indicate_state]]
...
if settings_file_check == false then
  file_is_on_start = 0
  Msg('NO FILE!')
  --[[conf_var.stop_txt, conf_var.pause_txt, conf_var.play_txt, conf_var.rec_txt = 'STOP', 'PAUSE', 'PLAY', 'RECORD' --texts
  
  conf_var.R_stop_bg,    conf_var.G_stop_bg,   conf_var.B_stop_bg   =  30,  30,  30 --Stop color BG (RGB)
  conf_var.R_pause_bg,   conf_var.G_pause_bg,  conf_var.B_pause_bg  =  30,  30,  30 --Pause color BG (RGB)
  conf_var.R_play_bg,    conf_var.G_play_bg,   conf_var.B_play_bg   =  30,  30,  30 --Play color BG (RGB)
  conf_var.R_rec_bg,     conf_var.G_rec_bg,    conf_var.B_rec_bg    = 200,  70,  70 --Rec color BG (RGB)
  
  conf_var.R_stop_txt,   conf_var.G_stop_txt,   conf_var.B_stop_txt  = 100, 100, 100 --Stop color TXT (RGB)
  conf_var.R_pause_txt,  conf_var.G_pause_txt,  conf_var.B_pause_txt = 100, 170, 190 --Pause color TXT (RGB)
  conf_var.R_play_txt,   conf_var.G_play_txt,   conf_var.B_play_txt  =  75, 170,  75 --Play color TXT (RGB)
  conf_var.R_rec_txt,    conf_var.G_rec_txt,    conf_var.B_rec_txt   =  30,  30,  30 --Rec color TXT (RGB)
  
  conf_var.repeat_unique_color_state = 0 -- enable repeat uniqur color (below)
  conf_var.repeat_on_color_state = 1     -- enable repeat ON uniqur color
  conf_var.repeat_off_color_state = 0    -- enable repeat OFF uniqur color
  conf_var.R_repeat_on, conf_var.G_repeat_on, conf_var.B_repeat_on    = 100, 170, 190 -- Repeat color ON
  conf_var.R_repeat_off, conf_var.G_repeat_off, conf_var.B_repeat_off =  35,  35,  35 -- Repeat color OFF
  
  conf_var.user_record_key_number = 42 -- * (num keyboard)
  conf_var.user_repeat_key_number = 47 -- / (num keyboard)
  conf_var.docker_start = 1 --start in docker
  
  conf_var.repeat_indicate_state = 1    -- eneble repeat indication]]
  ...
  --for i = 1, count_lines do
    --Msg(01)
  --  check_numb_string(settings_file)
  --end
  
  --Msg(conf_var['stop_txt'])
    --conf_var_values = {}
    --for _, v in ipairs(conf_var) do
    --  Msg(conf_var_values[v])
    --end
    --j = 1
    --for key, value in pairs(conf_var) do
  --Msg('k: '..key..'\tv: '..value)
        --string = check_numb_string(settings_file)
            
        --[[while string == nil or string == false do
         Msg('----------------------------')
          string = check_numb_string(settings_file)
        end]]
        --conf_var[key] = string
        --conf_var_values[j] = value
        --Msg('conf_var_values: '..conf_var[value])
        --j = j + 1
        --Msg(key)
        --Msg(string)
        --Msg('k:'..key..' v:'..value)
    ...
    --end
  --[[conf_var.stop_txt, conf_var.pause_txt, conf_var.play_txt, conf_var.rec_txt  =    settings_file:read(), settings_file:read(), settings_file:read(), settings_file:read() --texts
  conf_var.R_stop_bg,    conf_var.G_stop_bg,    conf_var.B_stop_bg   =   settings_file:read(), settings_file:read(), settings_file:read() --Stop color BG (RGB)
  
  conf_var.R_pause_bg,   conf_var.G_pause_bg,   conf_var.B_pause_bg  =   settings_file:read(), settings_file:read(), settings_file:read() --Pause color BG (RGB)
  conf_var.R_play_bg,    conf_var.G_play_bg,    conf_var.B_play_bg   =   settings_file:read(), settings_file:read(), settings_file:read() --Play color BG (RGB)
  conf_var.R_rec_bg,     conf_var.G_rec_bg,     conf_var.B_rec_bg    =   settings_file:read(), settings_file:read(), settings_file:read() --Rec color BG (RGB)
  
  conf_var.R_stop_txt,   conf_var.G_stop_txt,   conf_var.B_stop_txt  =   settings_file:read(), settings_file:read(), settings_file:read() --Stop color TXT (RGB)
  conf_var.R_pause_txt,  conf_var.G_pause_txt,  conf_var.B_pause_txt =   settings_file:read(), settings_file:read(), settings_file:read() --Pause color TXT (RGB)
  conf_var.R_play_txt,   conf_var.G_play_txt,   conf_var.B_play_txt  =   settings_file:read(), settings_file:read(), settings_file:read() --Play color TXT (RGB)
  conf_var.R_rec_txt,    conf_var.G_rec_txt,    conf_var.B_rec_txt   =   settings_file:read(), settings_file:read(), settings_file:read() --Rec color TXT (RGB)
  
  conf_var.repeat_unique_color_state = settings_file:read() -- enable repeat uniqur color (below)
  conf_var.repeat_on_color_state = settings_file:read()     -- enable repeat ON uniqur color
  conf_var.repeat_off_color_state = settings_file:read()    -- enable repeat OFF uniqur color
  conf_var.R_repeat_on,   conf_var.G_repeat_on,   conf_var.B_repeat_on    =  settings_file:read(), settings_file:read(), settings_file:read() -- Repeat color ON
  conf_var.R_repeat_off,  conf_var.G_repeat_off,  conf_var.B_repeat_off =  settings_file:read(), settings_file:read(), settings_file:read() -- Repeat color OFF
  
  conf_var.user_record_key_number = settings_file:read() -- * (num keyboard)
  conf_var.user_repeat_key_number = settings_file:read() -- / (num keyboard)
  conf_var.docker_start = settings_file:read() --start in docker
  
  conf_var.repeat_indicate_state = settings_file:read()
  --Msg('conf_var.repeat_indicate_state  '..conf_var.repeat_indicate_state)]]
...
--[[stop_txt, pause_txt, play_txt, rec_txt = conf_var.stop_txt, conf_var.pause_txt, conf_var.play_txt, conf_var.rec_txt
  
R_stop_bg,    G_stop_bg,    B_stop_bg   =  conf_var.R_stop_bg,    conf_var.G_stop_bg,   conf_var.B_stop_bg
R_pause_bg,   G_pause_bg,   B_pause_bg  =  conf_var.R_pause_bg,   conf_var.G_pause_bg,  conf_var.B_pause_bg
R_play_bg,    G_play_bg,    B_play_bg   =  conf_var.R_play_bg,    conf_var.G_play_bg,   conf_var.B_play_bg
R_rec_bg,     G_rec_bg,     B_rec_bg    = conf_var.R_rec_bg,     conf_var.G_rec_bg,    conf_var.B_rec_bg
  
R_stop_txt,   G_stop_txt,   B_stop_txt  = conf_var.R_stop_txt,   conf_var.G_stop_txt,   conf_var.B_stop_txt
R_pause_txt,  G_pause_txt,  B_pause_txt = conf_var.R_pause_txt,  conf_var.G_pause_txt,  conf_var.B_pause_txt
R_play_txt,   G_play_txt,   B_play_txt  =  conf_var.R_play_txt,   conf_var.G_play_txt,   conf_var.B_play_txt
R_rec_txt,    G_rec_txt,    B_rec_txt   =  conf_var.R_rec_txt,    conf_var.G_rec_txt,    conf_var.B_rec_txt 
  
repeat_unique_color_state = tonumber(conf_var.repeat_unique_color_state)
repeat_on_color_state = tonumber(conf_var.repeat_on_color_state)
repeat_off_color_state = tonumber(conf_var.repeat_off_color_state)
R_repeat_on, G_repeat_on, B_repeat_on    = conf_var.R_repeat_on, conf_var.G_repeat_on, conf_var.B_repeat_on
R_repeat_off, G_repeat_off, B_repeat_off =  conf_var.R_repeat_off, conf_var.G_repeat_off, conf_var.B_repeat_off
 
user_record_key_number = conf_var.user_record_key_number
user_repeat_key_number = conf_var.user_repeat_key_number
docker_start = tonumber(conf_var.docker_start)
  
local repeat_indicate_state = tonumber(conf_var.repeat_indicate_state)
 Msg(repeat_indicate_state)]]
 
 
--[[local stop_txt, pause_txt, play_txt, rec_txt = conf_var.stop_txt, conf_var.pause_txt, conf_var.play_txt, conf_var.rec_txt
  
local R_stop_bg,    G_stop_bg,    B_stop_bg   =  conf_var.R_stop_bg,    conf_var.G_stop_bg,   conf_var.B_stop_bg
local R_pause_bg,   G_pause_bg,   B_pause_bg  =  conf_var.R_pause_bg,   conf_var.G_pause_bg,  conf_var.B_pause_bg
local R_play_bg,    G_play_bg,    B_play_bg   =  conf_var.R_play_bg,    conf_var.G_play_bg,   conf_var.B_play_bg
local R_rec_bg,     G_rec_bg,     B_rec_bg    = conf_var.R_rec_bg,     conf_var.G_rec_bg,    conf_var.B_rec_bg
  
local R_stop_txt,   G_stop_txt,   B_stop_txt  = conf_var.R_stop_txt,   conf_var.G_stop_txt,   conf_var.B_stop_txt
local R_pause_txt,  G_pause_txt,  B_pause_txt = conf_var.R_pause_txt,  conf_var.G_pause_txt,  conf_var.B_pause_txt
local R_play_txt,   G_play_txt,   B_play_txt  =  conf_var.R_play_txt,   conf_var.G_play_txt,   conf_var.B_play_txt
local R_rec_txt,    G_rec_txt,    B_rec_txt   =  conf_var.R_rec_txt,    conf_var.G_rec_txt,    conf_var.B_rec_txt 
  
local repeat_unique_color_state = tonumber(conf_var.repeat_unique_color_state)
local repeat_on_color_state = tonumber(conf_var.repeat_on_color_state)
local repeat_off_color_state = tonumber(conf_var.repeat_off_color_state)
local R_repeat_on, G_repeat_on, B_repeat_on    = conf_var.R_repeat_on, conf_var.G_repeat_on, conf_var.B_repeat_on
local R_repeat_off, G_repeat_off, B_repeat_off =  conf_var.R_repeat_off, conf_var.G_repeat_off, conf_var.B_repeat_off
 
local user_record_key_number = conf_var.user_record_key_number
local user_repeat_key_number = conf_var.user_repeat_key_number
local docker_start = tonumber(conf_var.docker_start)
