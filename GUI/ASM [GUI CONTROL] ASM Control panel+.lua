...
function count_tracks(STATE, PARAM)
count_tracks_all =  reaper.CountTracks(0)
if not count_tracks_all then cheker() end --Cheker
  ...
  elseif LMB and mouse_on_button then --LMB
    ...
    asm.setColor(r-50, g-50, b-50, a)
      if indx_numb ~= indx_numb_old and button_up then --and changed_state then
        if get_start_indx_numb == false or start_indx_numb == nil then
          start_indx_numb = indx_numb
          get_start_indx_numb = true
        end
      ...
      elseif indx_numb ~= indx_numb_old then
        if start_indx_numb < indx_numb_old then
          q = indx_numb_old - start_indx_numb
          button_up = true
        
          for i = start_indx_numb, indx_numb_old do
            Msg(i)
            if func_01_LMB or func_02_LMB then
              if state_boolean and button_up then
                func_02_LMB(i)
          ...
          button_up = false
        elseif start_indx_numb > indx_numb_old then
          q = start_indx_numb - indx_numb_old
          button_up = true
          for i = start_indx_numb, indx_numb_old do
            Msg(i)
            if func_01_LMB or func_02_LMB then
    ...
    elseif indx_numb == indx_numb_old and button_up then
      ...
      elseif indx_numb ~= indx_numb_old then
        ...
        --b = 50
        --while b > 0 do
        --get_MDS = asm.get_mouse_down_state()
        --Msg('MGS  '..get_MDS)
        --b = b - 1
        --end
        button_up = true
    ...
    if start_indx_numb then
      Msg('--start_indx_numb'..start_indx_numb)
    end
    lmb_up = 1
  elseif www and LMB == false and lmb_up == 1 and mouse_on_button then
    if start_indx_numb then
      Msg('indx_numb'..indx_numb)
      Msg('start_indx_numb'..start_indx_numb)
      Msg('indx_numb_old'..indx_numb_old)
    end
    
    if start_indx_numb == nil then
      start_indx_numb = 0
    end
    if indx_numb_old == nil then
      indx_numb_old = 0
    end
...
------------------01 select-----------------
  ...
  function func_6_tr_sel_shift(track_numb) --LMB_Shift
    local sel_tracks = count_tracks('I_SELECTED', 1)
...
function set_01_tcp_hide() --LMB Show/hide tcp hide tracks
  ...
  count_tcp_show_tr = count_tracks('B_SHOWINMIXER', 1)
...
function set_01_mcp_hide() --LMB Show/hide tcp hide tracks
  ...
  count_mcp_show_tr = count_tracks('B_SHOWINTCP', 1)
