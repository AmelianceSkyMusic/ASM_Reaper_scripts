--[[
 * ReaScript Name: ASM [TRACK] Toggle FX (except VSTi) bypass/unbypass on selected tracks
 ...
 * REAPER: 5.0
 * Version: 1.0.0
 * Description: Toggle FX (except VSTi) bypass/unbypass on selected tracks
--]]
...
script_title = "ASM [TRACK] Toggle FX (except VSTi) bypass/unbypass on selected tracks"
...
local function MAIN()
  ...
  for i = 0, count_tracks_sel-1 do --do for all selected tracks
    ...
    for j = 0, count_FX-1 do --do for all FX on track
      ...
      if not is_VSTi then --is VSTi do this
        is_enable = reaper.TrackFX_GetEnabled(track_sel, j)
        if is_enable then
          reaper.TrackFX_SetEnabled(track_sel, j, 0) --set disable
        else
          reaper.TrackFX_SetEnabled(track_sel, j, 1) --set enable
        end
      end
