--[[
 * ReaScript Name: ASM [TRACK] Toggle FX (except VSTi) offline/online on selected tracks
 ...
 * REAPER: 5.0
 * Version: 1.0.0
 * Description: Toggle FX (except VSTi) offline/online on selected tracks
--]]
...
script_title = "ASM [TRACK] Toggle FX (except VSTi) offline/online on selected tracks"
    ...
    for j = 0, count_FX-1 do --do for all FX on track
      ...
      if not is_VSTi then --is VSTi do this
        ...
        if is_offline then
          ...
          reaper.TrackFX_SetOffline(track_sel, j, 1) --set offline
        end
      end
