--[[
 * ReaScript Name: ASM [TRACK] Toggle solo tracks (mute others) _soloed in place
 ...
 * REAPER: 5.0
 * Version: 1.0
 * Description: Soloing of selected tracks and automatic muting of other tracks (for visual perception). Toggle
--]]
...
--Solo type:
s_value = 2 -- 1=solo, 2=soloed in place

script_title = "ASM [TRACK] Toggle solo tracks (mute others) _soloed in place"
...
for i = 0, count_track_all - 1 do
    ...
    -- add S
    if not track_isSel and track_isSolo ~= 0 or (track_isSolo ~= 0 and track_isMute == 1) and checkpoint == 0 then
        reaper.SetMediaTrackInfo_Value(track_ud, "I_SOLO", s_value )
        reaper.SetMediaTrackInfo_Value(track_ud, "B_MUTE", 0)
        checkpoint = 1
    -- add M
    elseif not track_isSel and (track_isMute == 1 or track_isSolo == 0) and checkpoint == 0 then
